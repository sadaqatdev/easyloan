...
class SUnderReviewPage extends StatefulWidget {
  const SUnderReviewPage(this.map, {Key? key}) : super(key: key);
  final Map<String, dynamic> map;

  @override
  State<SUnderReviewPage> createState() => SUnderReviewPageState();
}

class SUnderReviewPageState extends State<SUnderReviewPage> {
  String? repayAmt;
  String? applyDate;
  ...
  _getAppSetting() async {
    try {
      Map<String, String> params = await CommonParams.addParams();
      params[Api.key] = "recentEverybodyLiveHusbandShe";
      ResultData resultData = await HttpManager.instance()
          .post(Api.getAppSetting(), params: params, mul: false);
      if (resultData.success) {
        Map<String, dynamic> map = resultData.data;
        String type = map["recentEverybodyLiveHusbandShe"] ?? '';
        if ("1" == type) {
          FlutterPlugin.openReview();
          LogUtil.platformLog(
              optType: PointLog.SYSTEM_GOOGLE_RATE_POPUP(), mul: false);
        }
      }
    } catch (e) {}
  }
